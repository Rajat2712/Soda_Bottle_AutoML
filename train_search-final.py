...
class CustomDatasetFromImages(Dataset):
    def __init__(self, csv_path):
        ...
        le.fit(self.data_info.iloc[:, 1])
        self.label_arr = np.asarray(le.transform(self.data_info.iloc[:, 1]))
    ...
    def __getitem__(self, index):
        ...
        img_as_img = img_as_img.resize((32,32), Image.ANTIALIAS)
...
def main():
  ...
  train_data = CustomDatasetFromImages("./train/Soda_bottles/train.csv")

  transformations = transforms.Compose([transforms.ToTensor()])

  valid_data = CustomDatasetFromImages("./test/Soda_bottles/test.csv")
  ...
  for epoch in range(args.epochs):
    ...
    #print(type(train_queue),type(search_queue))
    # training
...
def train(train_queue, search_queue, model, architect, criterion, optimizer, lr):
  ...
  for step, (input, target) in enumerate(train_queue):
    model.train()
    n = input.size(0)
    #print("typeeeeeeeeeeee",type(input),type(target))
